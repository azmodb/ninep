...
var fcallPool = &sync.Pool{New: func() interface{} { return &FcallBuffer{} }}

// FcallBuffer is a variable-sized buffer of bytes used to decode 9P2000
// messages.
type FcallBuffer struct {
	data    []byte
	err     error
	managed bool
}

// NewFcallBuffer creates and initializes a new FcallBuffer using data as
// its initial contents. The new Buffer takes ownership of data, and the
// caller should not use data after this call.
func NewFcallBuffer(data []byte) *FcallBuffer {
	return &FcallBuffer{data: data, managed: false}
}

func newFcallBuffer(size int, managed bool) (b *FcallBuffer) {
	if managed {
		b = fcallPool.Get().(*FcallBuffer)
	} else {
		b = &FcallBuffer{}
	}
...
func (b *FcallBuffer) Len() int { return len(b.data) }

// Release resets to be empty and releases the buffer.
func (b *FcallBuffer) Release() {
	if b == nil {
		return
	}
...
func (b *FcallBuffer) Err() error { return b.err }

const maxInt = int64(^uint(0) >> 1)

const bufBootstrapSize = 128

func (b *FcallBuffer) grow(n int) (off int) {
	if b.data == nil {
...
// false.
func (b *FcallBuffer) consume(n int) ([]byte, bool) {
	if b.err != nil {
		return nil, false
	}
	if len(b.data) < n {
...
func (b *FcallBuffer) uint64() uint64 {
	data, ok := b.consume(8)
	if !ok {
		return 0
	}
...
func (b *FcallBuffer) uint32() uint32 {
	data, ok := b.consume(4)
	if !ok {
		return 0
	}
...
func (b *FcallBuffer) uint16() uint16 {
	data, ok := b.consume(2)
	if !ok {
		return 0
	}
...
func (b *FcallBuffer) uint8() uint8 {
	data, ok := b.consume(1)
	if !ok {
		return 0
	}
...
func (b *FcallBuffer) string(max uint16, throw error) string {
	size := b.uint16()
	if size == 0 {
		return ""
	}
...
func (b *FcallBuffer) Tattach() (fid, afid uint32, uname, aname string) {
	fid = b.uint32()
	afid = b.uint32()
	uname = b.string(MaxUserNameSize, ErrUserNameTooLarge)
	aname = b.string(MaxPathSize, ErrPathTooLarge)
...
func (b *FcallBuffer) Rauth() (typ uint8, version uint32, path uint64) {
	typ = b.uint8()
	version = b.uint32()
	path = b.uint64()
	return
...
func (b *FcallBuffer) Rattach() (typ uint8, version uint32, path uint64) {
	typ = b.uint8()
	version = b.uint32()
	path = b.uint64()
	return
...
func (b *FcallBuffer) Rerror() string {
	return b.string(MaxEnameSize, ErrEnameTooLarge)
}

// Tflush decodes a Tflush message from Buffer.
func (b *FcallBuffer) Tflush() uint16 { return b.uint16() }

// Rflush decodes a Rflush message from Buffer.
//func (b *FcallBuffer) Rflush() uint16 { return b.uint16() }

// Twalk decodes a Twalk message from Buffer.
func (b *FcallBuffer) Twalk() (fid, newfid uint32, names []string) {
	fid = b.uint32()
	newfid = b.uint32()
	n := b.uint16()
...
func (b *FcallBuffer) Rwalk() (qids []Qid) {
	n := b.uint16()
	if n > MaxWalkElements {
		b.err = ErrWalkElements
		return
...
func (b *FcallBuffer) Topen() (fid uint32, mode uint8) {
	fid = b.uint32()
	mode = b.uint8()
	return
}
...
func (b *FcallBuffer) Ropen() (typ uint8, version uint32, path uint64, iounit uint32) {
	typ = b.uint8()
	version = b.uint32()
	path = b.uint64()
	iounit = b.uint32()
...
func (b *FcallBuffer) Tcreate() (fid uint32, name string, perm uint32, mode uint8) {
	fid = b.uint32()
...
func (b *FcallBuffer) Rcreate() (typ uint8, version uint32, path uint64, iounit uint32) {
	typ = b.uint8()
	version = b.uint32()
	path = b.uint64()
	iounit = b.uint32()
...
func (b *FcallBuffer) Tread() (fid uint32, offset uint64, count uint32) {
	fid = b.uint32()
	offset = b.uint64()
	count = b.uint32()
	return
...
func (b *FcallBuffer) Rread() (data []byte) {
	data = b.raw32()
	return
}

// Twrite decodes a Twrite message from Buffer.
func (b *FcallBuffer) Twrite() (fid uint32, offset uint64, data []byte) {
	fid = b.uint32()
	offset = b.uint64()
	data = b.raw32()
	return
...
func (b *FcallBuffer) Rwrite() (count uint32) {
	count = b.uint32()
	return
}

// Tclunk decodes a Tclunk message from Buffer.
func (b *FcallBuffer) Tclunk() (fid uint32) {
	fid = b.uint32()
	return
}

// Rclunk decodes a Rclunk message from Buffer.
//func (b *FcallBuffer) Rclunk() uint16 { return b.uint16() }

// Tremove decodes a Tremove message from Buffer.
func (b *FcallBuffer) Tremove() (fid uint32) {
	fid = b.uint32()
	return
}

// Rremove decodes a Rremove message from Buffer.
//func (b *FcallBuffer) Rremove() uint16 { return b.uint16() }

// Tstat decodes a Tstat message from Buffer.
func (b *FcallBuffer) Tstat() (fid uint32) {
	fid = b.uint32()
	return
}

// Rstat decodes a Rstat message from Buffer.
func (b *FcallBuffer) Rstat() (data []byte) {
	data = b.raw16()
	return
}

// Twstat decodes a Twstat message from Buffer.
func (b *FcallBuffer) Twstat() (fid uint32, data []byte) {
	fid = b.uint32()
	data = b.raw16()
	return
}
...
func (b *FcallBuffer) Rwstat() uint16 { return b.uint16() }
...
// Buffer.
func (d *Decoder) Next() (FcallType, uint16, *FcallBuffer, error) {
	size, typ, tag, err := d.decodeHeader()
	if err != nil {
		return 0, 0, nil, err
	}

	buf := newFcallBuffer(size, true)
